...
def normalize(s: str) -> str:
    s = s.lower()
    # Treat underscores as spaces before punctuation removal
    s = s.replace("_", " ")
    exclude = set(string.punctuation) - {"_"}  # keep '_' if you prefer
...
def eval_f1(prediction, answer):
    ...
    for p in prediction:
        for a in answer:
            if a not in used_answers and match(p, a):
                matched += 1
                used_answers.add(a)
                break  # stop after first match for this prediction
    precision = matched / len(prediction)
    recall = matched / len(answer)
    if precision + recall == 0:
        return 0, precision, recall
...
def eval_result(predict_file, cal_f1=True, topk = -1):
    ...
    eval_name = "detailed_eval_result_top_{topk}" if topk > 0 else 'detailed_eval_result'
    detailed_eval_file = predict_file.replace('predictions', eval_name)
    # Load results
    acc_list = []
    hit_list = []
    f1_list = []
