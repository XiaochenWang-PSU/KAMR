...
def safe_prompt(text: str) -> str:
    ...
    # Remove any characters that are not printable (including \u200b, BOMs, etc.)
    text = ''.join(c for c in text if c.isprintable())
    # Strip out any control characters not allowed in JSON
    text = re.sub(r'[\x00-\x1F\x7F]', '', text)
    # Force cast to string and encode/decode cleanly
...
class ChatGPT(BaseLanguageModel):
    ...
    def generate_sentence(self, dataset):
        s_time = time.time()
        res = []
        for llm_input in tqdm(dataset):
            # llm_input = safe_prompt(llm_input)
            ...
            # print("model_name =", repr(self.model_name))
